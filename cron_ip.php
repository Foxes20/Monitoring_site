...
/**
 * @var object $connect этa переменная из подключаемого файла db_service.php , в ней подключение к базе
 * @var integer $protocol этa переменная из селекта выбора протокола для проверки сайта
 */
set_time_limit(0);

//$result = mysqli_query($connect, "DELETE FROM `log`");
//exit;
    ...
    $rows = mysqli_fetch_all($result,MYSQLI_ASSOC );
        ...
        foreach ($rows as $row =>$key) {
            ...
            $resultLog = mysqli_query($connect, "SELECT * FROM `log` ");
            ...
            if (help($siteName, $protocol) == (200 <= 399)) {
                $status = 0;//"работает";
                $sql = mysqli_query($connect, "INSERT INTO `log` ( `date_log`, `status`,`id_form`) VALUES ('" . time() . "', '" . $status . "','" . $id_form . "' )");
